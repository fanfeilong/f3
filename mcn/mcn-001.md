...
windows内置curl了: http://daniel.haxx.se/blog/2018/01/13/microsoft-curls-too/ 一个命令行工具在所有主流操作系统里都支持，这非常不一样，从此它就是默认可用的。可惜了，wget没有同时入选，大概是curl功能上覆盖了wget吧，在没有默认内置之前，很多人更喜欢装wget，因为小巧。

## [C Data Structures and Algorithms]

* https://github.com/MichaelJWelsh/cdsa

This repository is an ongoing project of implementing generic data structures and algorithms in ANSI C. 这是github上一个纯C的通用数据结构库。如何实现纯C的数据结构实现可以同时阅读 [C语言接口与实现](http://book.douban.com/subject/1230040/) 这本书。
...
- [So you're thinking about investing in bitcoin? Don't ](http://theguardian.com/technology/2018/jan/15/should-i-invest-bitcoin-dont-mr-money-moustache)
...
## [MDN: learn]
Web开发的初学文档最佳的应该是MDN上的这个learn系列：https://developer.mozilla.org/en-US/docs/Learn 
...
听说过跨语言交互编程么？想自己尝试么？那么你可以的尝试组合如下：

1. 在C/C++语言里调用Lua脚本，C/C++和Lua脚本之间的交互。
2. 在C/C++语言里调用Python脚本，C/C++和Python脚本之间的交互。
3. 在C/C++语言里调用NodeJS脚本，C/C++和NodeJS脚本之间的交互。
4. 在C#语言里调用Lua脚本，C#语言和Lua脚本之间的交互。
5. 在Java语言里调用C/C++，通过JNI在Java和C/C++之间交互。
6. 在Android的WebView里加载HTML+JavaScript，在Android的Java和WebView的JS之间交互。


你可以搜索关键字，找到对应的教程自己学习，跨语言交互有一个专有名词叫foreign function interface (FFI)，这是FFI的wiki介绍：

* [FFI: en.wikipedia.org/wiki/Foreign_function_interface](https://en.wikipedia.org/wiki/Foreign_function_interface)

>A foreign function interface (FFI) is a mechanism by which a program written in one programming language can call routines or make use of services written in another.

* 从哪里开始？

一开始就弄js和c++会比较麻烦点，因为一般来说js库会用的是google 的v8引擎，你需要理解v8的c++接口先。而nodejs是在v8的基础上又开发了一层的。所以你要区分清楚：

1. 直接编译v8引擎的库，在c++里用v8引擎去跑javascript。
2. 给nodejs开发c++插件的方式去做c++和js的交互。

如果你想更快点体验如何在c/c++里使用脚本语言，可以从c里面调用lua开始，
lua语言的库和代码都相对简单点，lua本身就是被设计来嵌入在其他语言里用的。

lua本身只是一个被设计来可以轻松在其他语言里嵌入（一般来说是C），用来加载lua脚本，
并且可以方便的让宿主语言暴露接口给lua调用的。并没有限定lua会在哪里被使用。
例如魔兽世界游戏里就有大量使用lua脚本，很多游戏引擎都会选择lua做脚本语言。
也有很多GUI引擎选择用lua做脚本，让用户写GUI的交互事件逻辑。

* 做医疗开发的，是否用不上？

你全部用C/C++，C#也能搞定全部功能。但是你可能希望：

1. 简化代码的开发。
2. 设计一个框架，C/C++提供基础的库接口（不经常变动的部分）。
3. 在Lua脚本里写业务逻辑（经常变动的部分）。

那么，可能你开发了好多年后，厌恶了天天用C/C++写这些经常变动部分的代码，而且用C/C++写很简单的功能都需要很多代码，此时你的编程功力可能也够了，于是你尝试用一个脚本语言，例如Lua，嵌入你的框架，让C/C++提供核心类库功能，暴露给Lua，Lua里面使用这些C/C++暴露的框架接口，使用Lua相对简单的语法来编写新的业务代码。

所以说，这是一个构架设计的问题。

* 可是为什么Java里调用脚本语言的很少？

1. C/C++这种可以直接用指针的语言一般来说是应对业务逻辑开发不那么灵活，这个时候引入脚本语言会大大增加灵活性。
2. 但是像C#/Java这种语言本身就很灵活，此时引入脚本语言的动力不是那么大，但是如果你希望开发的应用程序能方便的做一些用户模块配置，插件开发，此时还是可能会选择嵌入脚本语言去实现。
3. 一方面脚本引擎的体积比较小，另一方面脚本代码环境里可以调用的接口可控（比如你可能不暴露任何访问本地文件的接口，达到隔离）。
4. 此外，当你需要在C#/Java里调用其他语言开发的模块，你就需要做跨语言交互，比如C#调用C/C++开发的Native模块，你需要使用.NET 的Platform Invoke（P/Invoke)方式去调用；比如Java调用C/C++开发的Native模块，你需要用JNI技术去调用，像Android上的开发大部分是用Java的，但是如果你要调用C/C++开发的模块，JNI是必须的。 
